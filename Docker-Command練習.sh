...
docker run --name phpmyadmin -d -p 8080:80 phpmyadmin
docker run --name phpmyadmin -d -e PMA_HOST=192.168.1.252 -p 8080:80 phpmyadmin


docker run --rm --name ps-server -e MYSQL_ROOT_PASSWORD=your_password `
  -v ${iniFile}:/tmp/config.ini -p 3306:3306 -d mysql:lts

docker run --name mysql-server -e MYSQL_ROOT_PASSWORD=pass -p 3306:3306 -d mysql

docker exec -it mysql-server mysql -u root -p
docker exec -t mysql-server mysqldump --help


docker run --name mysql-server -e MYSQL_ROOT_PASSWORD=pass -p 3306:3306 -d mysql
# -v .\_env\POS-Fengshan-sync.ini:/tmp/config.ini `
docker cp .\_env\POS-Fengshan-sync.ini mysql-server:/tmp/config.ini
docker exec -it mysql-server mysql --defaults-file=/tmp/config.ini

docker run --name mysql-server -e MYSQL_ROOT_PASSWORD=your_password -v .\_env\POS-Fengshan-sync.ini:/tmp/config.ini -p 3306:3306 -d mysql
docker exec -it mysql-server mysql --defaults-file=/tmp/config.ini

docker run --rm --name mysql-server -e MYSQL_ROOT_PASSWORD=your_password `
  -v mysql-config-vol:/tmp `
  -v ./_env/POS-Fengshan-sync.ini:/tmp/config.ini:ro `
  -p 3306:3306 -d mysql
